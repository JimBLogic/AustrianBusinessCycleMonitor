...
const fallback: Data = {
  ...
  cache: { generatedAt: new Date(0).toISOString(), validUntil: new Date(0).toISOString(), ttlSeconds: 900, mode: "shared-snapshot" },
  series: {},
  latest: {},
  bitcoin: { price: null, change24h: null, marketCap: null, supply: 0, stockToFlow: 0, blockHeight: null, hashRate: null, difficulty: null, feeFast: null, feeHour: null },
  derived: {
    changes: {},
...
const text = {
  en: {
    ...
    live: "LIVE MACRO TERMINAL", title: "The cycle, decoded.", subtitle: "Official data. Austrian interpretation. Cypherpunk skepticism.",
  ...
  es: {
    ...
    live: "TERMINAL MACRO EN VIVO", title: "El ciclo, descifrado.", subtitle: "Datos oficiales. Interpretación austriaca. Escepticismo cypherpunk.",
...
export default function Monitor() {
  ...
  const snapshotDate = data.provenance.mode === "fallback" ? null : new Date(data.requestedAt);
  const snapshotAgeMinutes = snapshotDate && Number.isFinite(snapshotDate.getTime())
  ...
  const snapshotTimestamp = snapshotDate && Number.isFinite(snapshotDate.getTime())
  ...
  const nextRefreshDate = data.cache?.validUntil
    ? new Date(data.cache.validUntil)
    : snapshotDate ? new Date(snapshotDate.getTime() + 900_000) : null;
  const nextRefreshTimestamp = nextRefreshDate && Number.isFinite(nextRefreshDate.getTime())
    ? new Intl.DateTimeFormat(lang === "es" ? "es-ES" : "en-GB", {
      dateStyle: "medium", timeStyle: "medium", timeZone: "Europe/Madrid",
    }).format(nextRefreshDate)
    : "—";
  const secondsToRefresh = nextRefreshDate && Number.isFinite(nextRefreshDate.getTime())
  ...
  return (
    <main>
      ...
      <section className="hero" id="top">
        <div className="eyebrow"><span className={`live-dot ${loading ? "pulse" : ""}`} /> {t.live} · {data.provenance.mode === "live" ? "LIVE" : data.provenance.mode === "stale-persisted" ? (lang === "es" ? "ÚLTIMO DATO VÁLIDO" : "LAST VALID DATA") : (lang === "es" ? "ESPERANDO FUENTES" : "AWAITING SOURCES")}</div>
        <div className="hero-grid">
          <div>
            ...
            <div className={`snapshot-status ${snapshotNeedsRefresh ? "needs-refresh" : "current"}`} role="status" aria-live="polite">
              ...
              <p>{snapshotAgeMinutes == null
                ? (lang === "es" ? "Todavía no hay un snapshot válido. Pulsa «Actualizar datos» para consultar las fuentes ahora." : "There is no valid snapshot yet. Press “Refresh data” to query sources now.")
          ...
          <article className="regime-card">
            ...
            <small>{t.updated}: {data.provenance.mode === "fallback" ? "—" : `${new Date(data.requestedAt).toISOString().replace("T"," ").slice(0,19)} UTC`} · {data.provenance.fredAvailable ?? 0}/{data.provenance.fredTotal ?? 0} {lang === "es" ? "SERIES MACRO" : "MACRO SERIES"}</small>
          </article>
        </div>
      </section>
      ...
      <section className="proof-section" aria-labelledby="proof-title">
        ...
        <div className="proof-grid">
          <a href="/api/health" target="_blank" rel="noreferrer"><span>01 · {lang === "es" ? "ESTADO DE FUENTES" : "SOURCE HEALTH"}</span><strong>{data.provenance.fredAvailable ?? 0}/{data.provenance.fredTotal ?? 0}</strong><p>{lang === "es" ? "Cobertura y estado técnico de proveedores." : "Coverage and provider health."}</p><b>{lang === "es" ? "Comprobar" : "Inspect"} ↗</b></a>
          <a href="/api/data-manifest" target="_blank" rel="noreferrer"><span>02 · {lang === "es" ? "CONTRATO DE DATOS" : "DATA CONTRACT"}</span><strong>v1.0</strong><p>{lang === "es" ? "Entradas, fórmulas, pesos y límites." : "Inputs, formulas, weights and limits."}</p><b>{lang === "es" ? "Auditar" : "Audit"} ↗</b></a>
      ...
      <section className="engine-section" id="engine">
        ...
        <article className="cycle-methodology" id="cycle-methodology">
          <div className="method-head">
            <div>
              <span className="kicker">{lang === "es" ? "METODOLOGÍA ABIERTA · V1.0" : "OPEN METHODOLOGY · V1.0"}</span>
        ...
        <div className="backend-grid">
          ...
          <article className="freshness-panel">
            <div className="backend-title">
              <div><span className="kicker">{lang === "es" ? "BACKEND TEMPORAL" : "TEMPORARY BACKEND"}</span><h3>{lang === "es" ? "Estado de las fuentes" : "Source status"}</h3></div>
              <span className="backend-live">● {lang === "es" ? "ACTIVO" : "LIVE"}</span>
            </div>
            <p>{lang === "es"
              ? "Actualizar llama al agregador servidor sin caché. Bitcoin y su red se consultan al momento; cada serie macro conserva la fecha de publicación y la fuente efectiva."
              : "Refresh calls the server aggregator without cache. Bitcoin and its network are queried now; each macro series keeps its release date and effective source."}</p>
            ...
            <div className="freshness-list">
              {data.freshness.length ? data.freshness.map((source) => <a href={`https://fred.stlouisfed.org/series/${source.id}`} target="_blank" rel="noreferrer" key={source.id}>
                <span><i className={source.status === "live" ? "ok" : source.status === "last-known-good" ? "backup" : "fail"} />{source.id} · {seriesSource(data, source.key)}{source.status === "last-known-good" ? " · BACKUP" : ""}</span>
      ...
      <footer><span>{lang === "es" ? "ABCM · INTELIGENCIA MACRO DE CÓDIGO ABIERTO" : "ABCM · OPEN SOURCE MACRO INTELLIGENCE"}</span><span><a href={`/learn?lang=${lang}`} target="_blank" rel="noreferrer">{lang === "es" ? "Aprende ↗" : "Learn ↗"}</a> · JimBLogic · 2026 · <a href="https://github.com/JimBLogic/AustrianBusinessCycleMonitor">GitHub ↗</a></span></footer>
